...
export const ReportWidget = ({ report, onAccept, onReject }: ReportWidgetProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  const { postId, title, subject, type, reasons, courseId, yearId } = report;

  const SelectedLabel = LABELS[type as PostType] || LABELS.VID;
      ...
      {isOpen && (
        <div className="border-t border-gray-100 bg-gray-50/30">
          <div className="max-h-80 overflow-y-auto p-4">
            {reasons.map((r, idx) => <DocumentReport key={r.id ?? idx} {...r} />)}
          </div>
        </div>
      )}
    </div>
