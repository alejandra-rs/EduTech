...
export const AdminWidget = ({ 
  ...
  return (
    <div className="mb-4 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden w-full transition-all hover:border-blue-100">
      <div 
        className={`p-4 flex flex-col md:flex-row items-center md:items-center justify-between gap-4 ${collapsible ? 'cursor-pointer hover:bg-gray-50' : ''}`}
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0 w-full md:w-auto justify-end pt-2 md:pt-0 border-t md:border-t-0">
          ...
          {actions.map((action, idx) => {
            ...
            if (isConfirming) {
              return (
                <div key={action.label} className="flex items-center gap-1 bg-gray-50 border rounded-lg p-1 animate-in slide-in-from-right-1" onClick={e => e.stopPropagation()}>
