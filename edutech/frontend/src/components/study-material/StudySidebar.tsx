...
const StudySidebar = ({ show, onToggle, stats, actions = [], navTitle, navItems = [], activeId }: StudySidebarProps) => (
  <aside className={`fixed right-0 top-0 h-full bg-gray-50 border-l border-gray-200 transition-all duration-300 z-50 shadow-2xl ${show ? "w-72" : "w-0"}`}>
    ...
    >
      <ChevronLeftIcon className={`size-5 text-gray-500 transition-transform ${show ? "rotate-180" : ""}`} />
    </button>
    ...
    <div className={`flex flex-col h-full p-5 mt-3 overflow-hidden transition-opacity ${show ? "opacity-100" : "opacity-0"}`}>
      ...
      {actions.length > 0 && (
        <div className="mt-6 space-y-2">
          {actions.map(({ label, icon: Icon, onClick, className }) => (
            <button type="button" key={label} onClick={onClick} className={className}>
      ...
      <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mt-10 mb-4">{navTitle}</h2>
      <nav className="flex-1 overflow-y-auto space-y-1">
        {navItems.map((item) => (
          ...
          >
            {item.icon
              ? <item.icon className={`size-4 mt-0.5 flex-shrink-0 ${item.status === undefined ? "text-gray-300" : item.status ? "text-green-500" : "text-red-500"}`} />
              : <div className={`size-2 rounded-full flex-shrink-0 ${item.status === undefined ? "bg-gray-200" : item.status ? "bg-green-500" : "bg-red-500"}`} />
            }
