...
export interface QuizCardProps {
  ...
  isCorrected: boolean;
  selectedIds: number[];
  onSelect: (qId: number, ansId: number, isMultiple: boolean) => void;
  onReset: (qId: number) => void;
}

const QuizCard = ({ question, onAnswered, isCorrected, selectedIds, onSelect, onReset }: QuizCardProps) => {
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm transition-all hover:shadow-md">
      ...
      <div className="space-y-2">
        {question.answers.map((ans) => {
          ...
          if (isCorrected) {
            if (ans.is_correct) statusStyle = "border-green-400 bg-green-50/50 text-green-900";
            else if (isSelected && !ans.is_correct) statusStyle = "border-red-300 bg-red-50/50 text-red-900";
