...
const QuizAnswer = ({ value, onChange, onDelete, isCorrect, onToggleCorrect, canDelete = true }: QuizAnswerProps) => (
  <div className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-all duration-200 group ${
    ...
    <button type="button"
      ...
      className={`size-5 rounded border flex items-center justify-center transition-all active:scale-75 flex-shrink-0 ${
        isCorrect ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 bg-gray-50 hover:border-green-400'
      }`}
    >
      {isCorrect && <CheckIcon className="size-3 stroke-[4px]" />}
    </button>
    ...
    {canDelete && (
      <button type="button" onClick={onDelete} className="text-gray-300 p-1 rounded hover:text-red-500 hover:bg-red-50 transition-all flex-shrink-0">
