...
export function ChatbotHeader({ lista, seleccionado, onCambiar, onClose, isDeepThinking, setIsDeepThinking }) {
              ...
              {lista.map((agente) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm hover:bg-blue-50 transition-colors ${
                    seleccionado.id === agente.id ? 'bg-blue-50 text-blue-700 font-semibold' : ''
                  }`}
                >
                  {agente.label}
                </button>
