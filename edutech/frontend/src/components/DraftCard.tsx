...
import type { Draft, DraftFlashcard, DraftQuiz } from '../models/documents/draft.models';

const TYPE_LABELS: Record<string, string> = { QUI: "Cuestionario", FLA: "Flashcards" };
...
const itemLabel = (draft: Draft): string => {
  if (draft.post_type === "FLA") {
    const count = (draft as DraftFlashcard).fla?.cards?.length ?? 0;
    return `${count} tarjeta${count !== 1 ? "s" : ""}`;
  }
  if (draft.post_type === "QUI") {
    const count = (draft as DraftQuiz).qui?.questions?.length ?? 0;
...
export default function DraftCard({ draft, deleting, onDelete }: DraftCardProps) {
