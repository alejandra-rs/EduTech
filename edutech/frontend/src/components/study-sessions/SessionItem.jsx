...
const SessionItem = ({ session, currentUserId }) => {
  ...
  const creatorName = `${session.creator?.first_name} ${session.creator?.last_name}` || session.creator?.username || "Anónimo";
  const creatorAvatar = session.creator?.profile_picture;
