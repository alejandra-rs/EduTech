...
export default function Header({
  ...
  return (
    <aside
      className={`h-screen bg-slate-900 flex flex-col py-8 shadow-2xl transition-all duration-500 ${isOpen ? "w-64 px-4" : "w-20 px-4"}`}
    >
      <NavLink
        key="/"
        to="/"
        className={`flex items-center gap-3 mb-10 text-white font-bold text-xl px-2 ${!isOpen && "justify-center"}`}
      >
      ...
      <NavLink
        to="/Profile"
        className={`flex items-center mb-10 p-2 rounded-2xl ${isOpen ? "bg-white/5 border border-white/10 gap-4" : "justify-center"}`}
      >
        ...
        {isOpen && (
          <div className="min-w-0">
            ...
            <p
              className={`text-xs font-semibold italic text-${isAdmin ? "red-400" : "gray-400"}`}
            >
      ...
      <nav className="flex flex-col gap-2 flex-1">
        ...
        {isOpen && (
          <button
            ...
            className={`${baseItemClass} text-red-400 hover:bg-red-500/10 mt-auto`}
          >
