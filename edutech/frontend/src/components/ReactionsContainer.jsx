...
import { getLikes, addLike, removeLike, getDislikes, addDislike, removeDislike } from '@services/connections';
...
const ReactionsContainer = ({ postId }) => {
  ...
  const handleLike = async () => {
    ...
    try {
      if (isLiked) {
        ...
        await removeLike(prev.likeRecordId);
      } else {
        ...
        if (isDisliked) {
          ...
          await removeDislike(prev.dislikeRecordId);
        }
  ...
  const handleReportSubmit = async ({ reasonId, description }) => {
    await createReport(postId, userData.id, reasonId, description);
    setIsReported(true);
  };
