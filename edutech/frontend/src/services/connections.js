...
export const isSubscribed = async (userId, courseId) => {
  try {
    ...
    console.error("Error en isSubscribed:", error);
    throw error;
  }
};
...
export const unsubscribe = async (userId, courseId) => {
  try {
    const deleteResponse = await fetch(`${BASE_URL}/courses/sub/?user=${userId}&course=${courseId}`, {
      method: "DELETE" 
    });
