...
def notify_subscribers_of_new_post(post):
    ...
    for subscription in subscriptions:
        ...
        try:
            msg = EmailMultiAlternatives(subject, text_body, from_email, [student.email])
            msg.attach_alternative(html_body, 'text/html')
            msg.send()
        except Exception:
            logger.exception('Failed to send notification to %s for post %s', student.email, post.pk)