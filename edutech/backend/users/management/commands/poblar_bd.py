        ...
        with open(ruta_json, "r", encoding="utf-8") as file:
            ...
            for c in datos.get("courses", []):
                ...
                Course.objects.get_or_create(
                    name=c["name"], year=y_obj, semester=c["semester"]
                )
            ...
            for p_data in datos.get("posts", []):
                try:
                    ...
                    if p_data["post_type"] == "PDF":
                        PDFAttachment.objects.get_or_create(
                            post=post, defaults={"file": p_data["content_url"]}
                        )
                    elif p_data["post_type"] == "VID":
                        YoutubeVideo.objects.get_or_create(
                            post=post, defaults={"vid": p_data["content_url"]}
                        )

                except Exception as e:
                    self.stdout.write(
                        self.style.WARNING(f"⚠️ Error en post {p_data['title']}: {e}")
                    )
            ...
            for c_data in datos.get("comments", []):
                ...
                if post:
                    Comment.objects.get_or_create(
                        user=user, post=post, message=c_data["message"]
                    )
