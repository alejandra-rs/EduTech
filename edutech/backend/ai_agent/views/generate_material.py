...
class GenerateMaterial(APIView):
    ...
    def post(self, request):
        ...
        if not docs:
            return Response(
                {"error": "No se encontró contexto suficiente..."}, status=404
            )
        ...
        try:
            ...
            if post_type == "QUI":
                ...
                if not lista_preguntas and (
                    "question" in json_data or "titulo" in json_data
                ):
                    lista_preguntas = [json_data]

                for q_data in lista_preguntas:
                    titulo_pregunta = q_data.get(
                        "question", q_data.get("titulo", "Pregunta sin título")
                    )
                    question = Question.objects.create(quiz=quiz, title=titulo_pregunta)

                    respuestas = q_data.get(
                        "answers", q_data.get("respuestas", q_data.get("opciones", []))
                    )
                    ...
                    for a_data in respuestas:
                        Answer.objects.create(
                            ...
                            is_correct=a_data.get(
                                "is_correct", a_data.get("es_correcta", False)
                            ),
                        )
                ...
                lista_flashcards = json_data.get(
                    "flashcards", json_data.get("cards", [])
                )

                if not lista_flashcards and (
                    "question" in json_data or "pregunta" in json_data
                ):
                    lista_flashcards = [json_data]

                for c_data in lista_flashcards:
                    texto_pregunta = c_data.get(
                        "question",
                        c_data.get(
                            "pregunta", c_data.get("front", "Tarjeta sin título")
                        ),
                    )
                    texto_respuesta = c_data.get(
                        "answer",
                        c_data.get("respuesta", c_data.get("back", "Sin respuesta")),
                    )

                    FlashCard.objects.create(
                        deck=deck, question=texto_pregunta, answer=texto_respuesta
                    )
            ...
            return Response(
                {"error": "Fallo al guardar el borrador en la base de datos."},
                status=500,
            )
