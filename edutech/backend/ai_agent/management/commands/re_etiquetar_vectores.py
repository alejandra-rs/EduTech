...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with engine.connect() as conn:
            self.stdout.write(
                self.style.WARNING(
                    "Recuperando todos los vectores de la base de datos..."
                )
            )
            ...
            if total == 0:
                self.stdout.write(
                    self.style.ERROR("No hay vectores en la base de datos.")
                )
                return

            self.stdout.write(
                self.style.WARNING(
                    f"Comenzando limpieza y etiquetado de {total} vectores..."
                )
            )
