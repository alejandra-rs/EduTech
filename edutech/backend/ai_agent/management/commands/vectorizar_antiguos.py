...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if total == 0:
            self.stdout.write(self.style.SUCCESS('¡Todo al día! No hay PDFs pendientes de vectorizar.'))
            return

        self.stdout.write(self.style.WARNING(f'Se han encontrado {total} PDFs pendientes. Iniciando...'))

        for index, pdf in enumerate(pdfs_pendientes, start=1):
            self.stdout.write(f'[{index}/{total}] Procesando PDF ID: {pdf.id} (Curso: {pdf.post.course.name})...')
            
            try:
                ingerir_nuevo_documento(pdf)
                self.stdout.write(self.style.SUCCESS(f'  -> ¡Vectorizado con éxito!'))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'  -> Error en PDF {pdf.id}: {str(e)}'))
