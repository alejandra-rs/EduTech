...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        self.stdout.write(
            self.style.WARNING("Conectando con la base de datos y modelos...")
        )
            ...
            try:
                res = ollama.chat(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": 'Responde JSON: {"pide_codigo": bool}',
                        },
        ...
        search_kwargs = {
            "k": 6
        }  # Bajamos el K base a 6 para dejar hueco a los de código
        ...
        while True:
            pregunta_raw = input("\n📝 Tú: ")
            if pregunta_raw.lower() in ["salir", "exit"]:
                break
            ...
            if mencion_con_comillas:
                ...
                pregunta_busqueda = pregunta_raw.replace(
                    mencion_con_comillas.group(0), ""
                ).strip()
                self.stdout.write(
                    self.style.WARNING(
                        f"🎯 Modo precisión: Buscando solo en '{titulo_real}'"
                    )
                )
            ...
            if es_pregunta_de_codigo_con_ia(pregunta_busqueda):
                ...
                # Búsqueda Texto (k=6)
                res_texto = vector_store.similarity_search(
                    pregunta_busqueda, k=6, filter=filtros
                )
                # Búsqueda Código (k=4)
                filtros_code = {**filtros, "tipo_contenido": "Code"}
                res_code = vector_store.similarity_search(
                    pregunta_busqueda, k=4, filter=filtros_code
                )
                documentos_recuperados = res_texto + res_code
            else:
                self.stdout.write("📖 Búsqueda estándar (10 Texto)")
                documentos_recuperados = vector_store.similarity_search(
                    pregunta_busqueda, k=10, filter=filtros
                )
            ...
            for i, d in enumerate(documentos_recuperados):
                ...
                mapa_vectores[ref_num] = (
                    d  # Guardamos el objeto LangchainDocument entero
                )
            ...
            respuesta = (prompts["prompt_estricto"] | llm).invoke(
                {"context": contexto_str, "question": pregunta_busqueda}
            )
                    ...
                    json_referencias.append(
                        {
                            ...
                            "doc_id": doc_id_real,
                            "pagina": pagina,
                            "cita_exacta": texto_del_vector[:200]
                            + "...",  # Guardamos los primeros 200 caracteres de la fuente
            ...
            self.stdout.write(
                self.style.WARNING(f"📄 Revisa el archivo: {nombre_archivo}")
            )
